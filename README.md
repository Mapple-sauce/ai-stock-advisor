...
系统共包含 **8 个 AI Agent**，分为三层：

#### 第一层：个股深度分析层（4 Agent 并行）
用于 `analyze` 模式，对单只股票做全维度扫描：

| Agent | 角色 | 输入数据 | 分析技能 | 输出 |
|-------|------|----------|----------|------|
| **StockAnalyst** 🧠 | 首席分析师 | 行情/K线/技术指标/财务/新闻/资金流 + 行业背景 | MA趋势、MACD、RSI、KDJ、布林带、量价、估值、财务、情绪、风险评估、后市展望 | 结构化 JSON（评分1-100+评级+买卖建议+止损） |
| **SectorAnalyst** 🏭 | 行业板块专家 | 板块指数涨跌幅/领涨跌股/成交额 | 板块趋势判断、驱动因素（政策/周期/事件）、板块热度、龙头股表现、未来1-3月展望 | Markdown 报告 |
| **IndustryPositionAnalyst** 🏆 | 行业地位专家 | 同业可比公司列表/市值排名/PE排名 | 市场地位（龙头/跟随者）、估值对比、成长性对比、竞争优势（护城河）、行业格局 | Markdown 报告 |
| **SupplyChainAnalyst** 🔗 | 供应链专家 | 行业供应链知识（上游/下游/典型风险） | 上游依赖度、下游客户结构、产业链议价能力、供应链风险（地缘/涨价/技术封锁） | Markdown 报告 |

> ⚡ 这 4 个 Agent 通过 `ThreadPoolExecutor` **并行执行**，总耗时 ≈ 单个最慢 Agent 的时间。

#### 第二层：买卖决策层（5 Agent 协作）
用于 `buy` / `sell` 模式，模拟投资分析团队：

| Agent | 角色 | 分析技能 | 输出 |
|-------|------|----------|------|
| **TechnicalAnalyst** 📊 | 技术分析专家 | 趋势判断（均线多头/空头）、动量（MACD金叉死叉）、RSI超买超卖、KDJ信号、布林带位置、量价关系、形态识别（突破/双底） | Markdown 技术报告 |
| **FundamentalAnalyst** 📋 | 基本面分析专家 | 估值水平（PE/PB/PS历史分位）、盈利能力（毛利率/净利率/ROE）、成长性（营收/利润增速）、财务健康（资产负债率/现金流） | Markdown 基本面报告 |
| **SentimentAnalyst** 📰 | 舆情分析专家 | 新闻情绪（正面/负面/中性）、事件影响力（业绩预告/重组/监管）、市场热点关注度、潜在黑天鹅 | Markdown 舆情报告 |
| **RiskAnalyst** ⚠️ | 风险评估专家 | 技术风险（高位/超买/顶背离）、基本面风险（估值泡沫/业绩下滑）、市场系统性风险、流动性风险、关键支撑压力位 | Markdown 风险报告 |
| **ChiefAgent** 🧠 | 首席投资官 | 综合上述4位分析师报告 → 加权评分 → 分歧点独立判断 → 最终买入/卖出决策 | 结构化 JSON（评分1-10+操作建议+仓位+止损+持有周期） |

> 📋 **买入决策流程**: Technical → Fundamental → Sentiment → Risk → Chief（顺序调用）
> 📋 **卖出决策流程**: Technical → Sentiment → Risk → Chief（不含基本面）
...
| Agent | 角色 | 分析技能 | 输出 |
|-------|------|----------|------|
| **MarketScanner** 🔍 | 市场扫描专家 | 从候选股中精选（低位潜力股/强势突破股），结合10因子评分+AI判断 | Markdown 精选列表 |
| **HunterAgent** 🎯 | 低位挖掘专家 | K线底部形态（双底/W底）、地量见地价、RSI超卖回升、MACD零轴下方金叉、基本面催化因素 | Markdown 深度挖掘报告 |

### 数据来源与工具链

| 数据/工具 | 用途 | 使用方 |
|-----------|------|--------|
| **Baostock** | K线数据、行业分类 | 所有 Agent |
| **新浪财经 (akshare)** | 实时行情 | 所有 Agent |
| **东方财富 (akshare)** | 板块指数、ETF行情 | SectorAnalyst |
| **akshare 基本面** | 财务数据、估值、现金流 | StockAnalyst, FundamentalAnalyst |
| **akshare 新闻/研报** | 新闻、券商研报 | SentimentAnalyst, StockAnalyst |
| **akshare 资金流向** | 主力净流入、大单统计 | StockAnalyst |
| **matplotlib** | 图表生成（价格/技术/回撤） | StockAnalyst |
| **fpdf2** | PDF 报告生成 | 报告模块 |
| **concurrent.futures** | 4 Agent 并行执行 | StockAnalyst |
